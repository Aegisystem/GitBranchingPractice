...
async function hasCompletedFeedback(api, issueNumber, missionId) {
  ...
  const feedbackMarker = `<!-- ${PRACTICE_MARKER}:auto-feedback:mission=${missionId} -->`;
...
Este issue fue cerrado por ${mention}, pero las misiones de esta práctica solo se cierran cuando el workflow **Validar progreso de misiones** verifica el criterio correspondiente.
...
El workflow dejó el issue cerrado nuevamente para conservar el avance real de la práctica.`;
}
...
async function main() {
  ...
  if (!missionId) {
    console.log("El issue no pertenece a la práctica guiada. No se modifica.");
    return;
  }
